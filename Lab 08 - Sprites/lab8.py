...
class MyGame(arcade.Window):
    ...
    def setup(self):
        ...
        for i in range(BULLETS_COUNT):
            ...
            bullets = Bullets("ammo_machinegun.png", SPRITE_SCALING_BULLETS)

            # Position the bullets
            bullets.center_x = random.randrange(SCREEN_WIDTH)
            bullets.center_y = random.randrange(SCREEN_HEIGHT)
            bullets.change_x = random.randrange(-3, 4)
            bullets.change_y = random.randrange(-3, 4)

            # Add the bullets to the lists
            self.bullets_list.append(bullets)
